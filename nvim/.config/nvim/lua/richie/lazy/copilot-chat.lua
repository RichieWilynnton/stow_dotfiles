			...
			vim.api.nvim_create_user_command("CopilotChatInline", function(args)
				chat.ask(args.args, {
					...
					window = {
						...
						height = 0.4,
						row = 1,
						border = "single", -- Add border
						focusable = true, -- Ensure it can be focused
					},
				})
			end, { nargs = "*", range = true })
		...
		keys = {
			...
			},
			{
				...
				function()
					local chat = require("CopilotChat")
					local select = require("CopilotChat.select")
					chat.ask("Please fix the diagnostic error in this code.", {
						selection = select.visual or select.buffer,
					})
				end,
				mode = { "n", "v" },
				desc = "CopilotChat - Fix Diagnostic with Selection",
			},
